...
const Mt203Page = () => {
  ...
  useEffect(() => {
    fetchMessagesByType('MT203', page - 1, pageSize);
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h6">
          MT203 Messages ({messagePage.totalElements || 0})
        </Typography>
      ...
      {messagePage.content.length === 0 && !loading && (
        <Alert severity="info">No MT203 messages found.</Alert>
      )}

      {messagePage.content.length > 0 && !loading && (
        <>
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="MT203 messages table">
              <TableHead>
                <TableRow>
